...
void  COutBufPacket::RemovePacket()
{
    ...
    int nTimeDiff = (nowtime.tv_sec - m_lastTime.tv_sec)*1000 + (nowtime.tv_usec - m_lastTime.tv_usec)/1000;
	if(nTimeDiff > 1000)
	{
		m_OutLock.Lock();
		...
		while(m_mapOutPutIter != m_mapOutPutPacket.end())
		{
			...
			unsigned int dwDiff = (nowtime.tv_sec - m_mapOutPutIter->second->getTimePts()) * 1000;
			if(dwDiff > 2000)
			{
