...
CRTMPProcess::CRTMPProcess():CNetProcess()
{
	...
	m_bDoPause = true;
    m_dwLastConnectTime.tv_sec = 0;
    m_dwLastConnectTime.tv_usec = 0;
	InitRTMPAddlist();
}
...
int CRTMPProcess::ReadMedia(unsigned char* pBuf,unsigned nBufSize)
{
	...
	{ 
		if(m_status == RTMP_PLAY || m_status == RTMP_OPEN)
		{
			...
			{
                struct timeval dwCur;
                gettimeofday(&dwCur,NULL);
                
				int dwDiff = (dwCur.tv_sec - m_dwLastConnectTime.tv_sec);
				if(dwDiff > 10000)
				{
					rtmp_connect();
					m_dwLastConnectTime = dwCur;
				}
			}
			if(RTMP_IsConnected(m_prtmp))
			{
